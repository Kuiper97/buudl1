...
import os
#about version 
#make size of file less than 50Mb
...
def compress_pdf(input_file, output_file, max_size=50 * 1024 * 1024):
    """Compress PDF using PyMuPDF and ensure the output file is less than max_size."""
    
    # Initialize the size of the output file
    compressed_size = float('inf')  # Start with an infinitely large size
    max_cycles = 10  # Maximum number of compression attempts
    attempt = 0  # Counter for attempts

    # Loop until the output file is less than max_size or max attempts reached
    while compressed_size > max_size and attempt < max_cycles:
        # Open the original PDF
        doc = fitz.open(input_file)

        # Save the document with compression
        doc.save(output_file, garbage=4, deflate=True)  # First compression attempt
        doc.close()

        # Check the size of the compressed file
        compressed_size = os.path.getsize(output_file)

        # If the size is acceptable, break the loop
        ...
        # If still too large, print a warning
        print(f"Compressed file size {compressed_size / (1024 * 1024):.2f} MB exceeds 50 MB. Attempt {attempt} of {max_cycles}...")

        # Optionally, you could adjust the compression settings here
        # For example, you might want to adjust the quality or DPI if applicable

        # Save again with garbage collection and deflation
        # This may not change the compression, but it's worth a try
        temp_output_file = output_file + ".temp"
        doc = fitz.open(output_file)
        doc.save(temp_output_file, garbage=4, deflate=True)
        doc.close()

        # Check the new size
        compressed_size = os.path.getsize(temp_output_file)

        # If the new size is acceptable, rename it to the output file
        if compressed_size <= max_size:
            os.rename(temp_output_file, output_file)
            break
        else:
            # Clean up the temporary file if still too large
            os.remove(temp_output_file)

    if compressed_size > max_size:
        print(f"Unable to compress file below {max_size / (1024 * 1024):.2f} MB after {max_cycles} attempts.")
    
    return compressed_size  # Return the final size of the compressed file



def merge_pdfs(pdf_files, page_ranges):
    """Merge PDFs and ensure final size is less than 50MB"""
    output_file = filedialog.asksaveasfilename(defaultextension='.pdf')
    if not output_file:
        return
    ...
    temp_output_file = output_file + ".temp"
    try:
        with open(temp_output_file, 'wb') as pdf_output:
            pdf_writer.write(pdf_output)

        # Loop to ensure final file size is less than 50 MB
        while os.path.getsize(temp_output_file) > 50 * 1024 * 1024:  # 50 MB
            # Compress the PDF to reduce its size
            compressed_output_file = output_file + ".compressed.pdf"
            compress_pdf(temp_output_file, compressed_output_file)

            # Check the size of the compressed file
            if os.path.getsize(compressed_output_file) <= 50 * 1024 * 1024:
                # Move the compressed file to the final output location
                os.rename(compressed_output_file, output_file)
                break
            else:
                # If still too large, show an error and allow user to adjust page ranges
                messagebox.showerror("Error", "The compressed PDF file still exceeds 50MB. Please reduce the number of pages.")
                os.remove(compressed_output_file)  # Clean up the compressed file
                os.remove(temp_output_file)  # Clean up the temporary file
                return

        # If the size is acceptable, move the temporary file to the final output location
        if os.path.getsize(temp_output_file) <= 50 * 1024 * 1024:
            os.rename(temp_output_file, output_file)
